...
from experiments.crazy_env.data_collection4 import Env
...
ARGUMENTS = [
    ...
    ["--start", int,90,"start model"],  #TODO
    ["--end", int,100, "end model"]  #todo
]
...
def test(arglist, log,full_load_dir,test_iteration):
    ...
    with U.multi_threaded_session() as sess:
        ...
        reader = pywrap_tensorflow.NewCheckpointReader(full_load_dir)
        var_to_shape_map = reader.get_variable_to_shape_map()
        # Print tensor name and value
        f = open('trained_params.txt', 'w')
        ...
        while iteration < arglist.test_time:
            ...
            if done or terminal:
                ...
                collect_frequency.append(np.sum(meaningful_get))
        ...
        indicator_to_pandas = [
            ...
            str(np.mean(collect_frequency)),
            str(np.mean(charge_frequency)),
            str(np.mean(station_remain))
        ]
...
if __name__ == '__main__':
    ...
    df=pd.DataFrame(columns=["test_model",
                             ...
                             "efficiency","e_min","e_max","collect","charge","station remain"])
    ...
    df.to_csv(arglist.load_dir+"/choose_best_model.csv",index=0)
    print('\n', 'TEST finished')
